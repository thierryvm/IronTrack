    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-2">
          ...
          >
            <Plus className="h-4 w-4" /> Lien externe
          </button>
        </div>
        <input
          ref={fileInputRef}
      ...
      {showAddExternal && (
        <div className="mb-4 flex flex-col sm:flex-row items-center gap-2 bg-blue-50 p-3 rounded-lg border border-blue-200">
          ...
          <button
            ...
            className="text-gray-600 hover:text-gray-700 dark:text-gray-300 text-xs ml-2"
          >Annuler</button>
        ...
        {sounds.map(sound => (
          <div key={sound.id} className={`flex flex-col gap-2 p-4 rounded-xl shadow-sm border border-[#E5E7EB] bg-[#F6F8FA] hover:shadow-md transition ${selectedSoundId === sound.id ? 'border-orange-600 bg-orange-100' : ''}`}
              ...
              {renamingId === sound.id ? (
                <div className="flex flex-col w-full gap-2">
                  <input
                    ...
                    className="border rounded-lg px-3 py-2 text-base w-full bg-white dark:bg-gray-900 focus:ring-2 focus:ring-orange-400 text-gray-900 dark:text-gray-100"
                    style={{fontSize:'1rem'}}
            ...
            <div className="flex flex-row flex-wrap gap-2 mt-4 w-full justify-center items-center min-h-[48px]">
              ...
              <button onClick={() => { setRenamingId(sound.id); setNewName(sound.name); }} className="text-yellow-500 hover:text-yellow-600 bg-white dark:bg-gray-900 border border-yellow-200 rounded-full p-3 shadow flex-shrink-0" title="Renommer" aria-label="Renommer le son" style={{minWidth:40, minHeight:40}}>
              ...
              <button onClick={() => handleDelete(sound)} className="text-red-500 hover:text-red-700 bg-white dark:bg-gray-900 border border-red-200 rounded-full p-3 shadow flex-shrink-0" title="Supprimer" style={{minWidth:40, minHeight:40}}>
