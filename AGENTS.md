...
## 🤖 Système d'Agents & Commandes Codex
...
### Agents de référence (`.Codex/agents/`)

| Agent         | Fichier                           | Usage                             |
| ------------- | --------------------------------- | --------------------------------- |
| Orchestrateur | `.Codex/agents/orchestrator.md`  | Coordination multi-agents         |
| UI/UX         | `.Codex/agents/ui-ux.md`         | Design system, tokens, composants |
| Code Review   | `.Codex/agents/code-review.md`   | Qualité, TypeScript, hooks        |
| Architecture  | `.Codex/agents/architecture.md`  | Next.js, SOLID, structure         |
| Performance   | `.Codex/agents/performance.md`   | Core Web Vitals, optimisation     |
| Accessibilité | `.Codex/agents/accessibility.md` | WCAG 2.2 AA                       |
| Sécurité      | `.Codex/agents/security.md`      | Supabase RLS, RGPD                |

### Slash Commands (`.Codex/commands/`)

| Fichier                             | Commande         | Description                          |
| ----------------------------------- | ---------------- | ------------------------------------ |
| `.Codex/commands/design-audit.md`  | `/design-audit`  | Audite design d'un fichier/page      |
| `.Codex/commands/code-review.md`   | `/code-review`   | Revue complète du code               |
| `.Codex/commands/fix-dark-mode.md` | `/fix-dark-mode` | Corrige incohérences dark mode       |
| `.Codex/commands/new-component.md` | `/new-component` | Crée un composant conforme           |
| `.Codex/commands/orchestrate.md`   | `/orchestrate`   | Agent maître — délègue selon demande |
...
### Règle UI complémentaire

- **⬆️ Bouton "remonter en haut" uniquement si utile** — seulement sur les pages longues où il apporte un vrai confort, jamais par défaut partout, et toujours en version discrète/non intrusive.
...
- **Gestion**: Via CLI `Codex mcp add/list/remove` (stocké dans `~/.Codex.json`)
...
- **Commandes**: `Codex mcp list` pour vérifier l'état, `Codex mcp add <nom> npx <package>` pour ajouter
...
- **Documentation officielle** : https://docs.anthropic.com/en/docs/Codex/overview
...
- **RÉPONDRE TOUJOURS EN FRANÇAIS** : Codex doit répondre uniquement en français pour tous les projets IronTrack
