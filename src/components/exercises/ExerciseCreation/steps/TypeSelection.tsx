...
function TypeCard({ option, selected, onSelect}: TypeCardProps) {
 ...
 :'border-border bg-card hover:border-red-300 hover:bg-red-50'
}`}
 >
 <div className={`p-2 rounded-lg ${
 selected 
 ? color ==='orange' ?'bg-primary' :'bg-red-500'
 :'bg-gray-100'
}`}>
 ...
 initial={{ scale: 0}}
 animate={{ scale: 1}}
 className={color ==='orange' ?'text-orange-800' :'text-safe-error'}
 >
 <div className="w-6 h-6 bg-current rounded-full flex items-center justify-center">
 <div className="w-2 h-2 bg-card border border-border rounded-full" />
 </div>
 </motion.div>
 )}
 </motion.button>
