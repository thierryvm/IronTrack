...
jest.mock('@/components/exercises/ExercisePhotoUpload', () => ({
  ExercisePhotoUpload: ({ onPhotoUploaded, onPhotoRemoved, currentPhoto, disabled }: unknown) => (
...
jest.mock('@/components/ui/input', () => ({
  Input: ({ value, onChange, placeholder, className, ...props }: unknown) => (
    <input
      value={value}
      onChange={onChange}
...
jest.mock('@/components/ui/button', () => ({
  Button: ({ children, onClick, loading, disabled, icon, variant, size, fullWidth, className, ...props }: unknown) => (
    <button
      onClick={onClick}
...
jest.mock('@/components/ui/textarea', () => ({
  Textarea: ({ value, onChange, placeholder, readOnly, variant, rows, autoResize, ...props }: unknown) => (
    <textarea
      value={value}
      onChange={onChange}
      placeholder={placeholder}
...
jest.mock('@/components/ui/ButtonMigrated', () => ({
  ButtonMigrated: ({ children, onClick, loading, disabled, icon, variant, size, fullWidth, className, ...props }: unknown) => (
    <button
      onClick={onClick}
      disabled={disabled || loading}
      className={className}
...
describe('ExerciseEditForm2025 - Tests de validation critiques', () => {
  ...
  describe('📱 TESTS UI/UX', () => {
    test('Loading state initial', () => {
      ...
      const spinner = document.querySelector('.w-8.h-8.border-2.border-orange-600')
