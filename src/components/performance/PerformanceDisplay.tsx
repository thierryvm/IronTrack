...
export function PerformanceDisplay({
 ...
 switch (variant) {
 case'card':
 return'bg-background rounded-lg p-4 border border-border'
 case'detailed':
 return'bg-card rounded-lg p-6 border border-border shadow-sm'
 case'compact':
 return'text-sm'
 default:
 return''
 ...
 return (
 <div 
 className={`text-gray-600 italic ${getVariantClasses()} ${className}`}
 data-testid={testId}
 >
 ...
 {showIcon && <TrendingUp className="h-6 w-6 text-orange-800 flex-shrink-0" />}
 ...
 <div className={`
 flex items-center 
 ${variant ==='detailed' ?'flex-wrap gap-2' :'space-x-2'}
 ${variant ==='compact' ?'text-xs' :'text-sm'}
 `}>
 {metrics.map((metric, index) => (
 <div key={index} className="flex items-center space-x-1 flex-shrink-0">
 {showIcon && variant !=='compact' && (
 <span className="text-gray-700">{metric.icon}</span>
 )}
 <span className={`
 font-medium
 ${variant ==='detailed' ?'text-foreground' :'text-gray-700'}
 `}>
 {metric.text}
 </span>
 ...
 <div className="mt-2 p-2 bg-background rounded text-xs text-gray-600">
