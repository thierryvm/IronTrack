...
export function StatCardSkeleton() {
  return (
    <div className="bg-white p-3 sm:p-6 rounded-xl shadow-sm animate-pulse min-h-[120px]">
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg min-h-[60px] animate-pulse">
...
export function ActionCardSkeleton({ count = 4 }: { count?: number }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="p-4 border-2 border-gray-100 rounded-xl min-h-[100px] animate-pulse">
