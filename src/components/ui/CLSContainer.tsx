...
export function StatCardSkeleton() {
 return (
 <div className="bg-card border border-border p-2 sm:p-6 rounded-xl shadow-sm animate-pulse min-h-[120px]">
...
export function ListItemSkeleton({ count = 3}: { count?: number}) {
 ...
 <div key={i} className="flex items-center justify-between p-2 bg-background rounded-lg min-h-[60px] animate-pulse">
...
export function ActionCardSkeleton({ count = 4}: { count?: number}) {
 ...
 {Array.from({ length: count}).map((_, i) => (
 <div key={i} className="p-4 border-2 border-border rounded-xl min-h-[100px] animate-pulse">
