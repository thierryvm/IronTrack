...
export default function SoundLibrary({ userId, selectedSoundId, onSoundAdded, onSoundDeleted}: SoundLibraryProps) {
 ...
 <span className="block text-sm font-medium text-gray-700">Bibliothèque de sons</span>
 <div className="flex gap-2">
 <button
 onClick={handleUploadClick}
 className="flex items-center gap-2 px-2 py-2 bg-primary hover:bg-primary-hover text-white rounded-lg font-semibold shadow transition-colors focus:outline-none focus:ring-2 focus:ring-orange-300"
 disabled={uploading}
 >
 ...
 className="bg-secondary hover:bg-secondary text-white px-2 py-2 rounded font-semibold"
 >Ajouter</button>
 <button
 onClick={() => { setShowAddExternal(false); setExternalUrl(''); setExternalName(''); setExternalError(null);}}
 className="text-gray-600 hover:text-gray-700 text-xs ml-2"
 >Annuler</button>
 {externalError && <span className="text-xs text-safe-error ml-2">{externalError}</span>}
 </div>
 )}
 <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
 {sounds.length === 0 && <div className="text-xs text-gray-600 col-span-full">Aucun son enregistré.</div>}
 {sounds.map(sound => (
 <div key={sound.id} className={`flex flex-col gap-2 p-4 rounded-xl shadow-sm border border-[#E5E7EB] bg-[#F6F8FA] hover:shadow-md transition ${selectedSoundId === sound.id ?'border-primary bg-orange-100' :''}`}
 style={{minWidth:0}}>
 ...
 className="border rounded-lg px-2 py-2 text-base w-full bg-card border border-border focus:ring-2 focus:ring-orange-400 text-foreground"
 style={{fontSize:'1rem'}}
 ...
 <button onClick={() => { setRenamingId(null); setNewName('');}} className="text-gray-700 text-xl" title="Annuler" style={{padding:'0 6px'}}><svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
 </div>
 </div>
 ) : (
 <>
 <span className="block text-base font-semibold text-foreground truncate mb-1">{sound.name}</span>
 </>
 )}
 </div>
 ...
 <button onClick={() => handleDelete(sound)} className="text-safe-error hover:text-red-700 bg-card border border-border border border-red-200 rounded-full p-2 shadow flex-shrink-0" title="Supprimer" style={{minWidth:40, minHeight:40}}>
