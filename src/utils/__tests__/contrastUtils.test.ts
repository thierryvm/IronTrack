...
describe('🛡️ Safe Colors - WCAG Compliance', () => {
 ...
 { name:'TEXT_PRIMARY', class:'text-foreground', expected: true},
 { name:'TEXT_SECONDARY', class:'text-gray-700', expected: true},
 { name:'TEXT_MUTED', class:'text-gray-600', expected: true},
 ...
 const classToHex = {
'text-foreground': TEST_COLORS.GRAY_900,
'text-gray-700': TEST_COLORS.GRAY_700,
'text-gray-600': TEST_COLORS.GRAY_600,
...
describe('🎨 Dynamic Class Generators', () => {
 ...
 expect(createSafeTextClass('gray-100','secondary')).toBe('text-card-foreground')
 ...
 expect(primaryButton).toContain('hover:bg-primary-hover')
 ...
 expect(smallButton).toContain('px-2 py-2 text-sm')
 
 const largeButton = createSafeButtonClass('primary','lg')
 expect(largeButton).toContain('px-6 py-2 text-lg')
})
...
describe('🚨 Development Validation', () => {
 ...
 validateContrastInDev('text-yellow-400','bg-card','TestComponent')
 
 // Comme text-yellow-400 n'est pas dans colorMap, aucun warning attendu
 // Test alternatif : vérifier qu'aucune erreur n'est jetée
 expect(() => {
 validateContrastInDev('text-unknown-color','bg-card','TestComponent')
}).not.toThrow()
 ...
 validateContrastInDev('text-foreground','bg-card','SafeComponent')
 ...
 validateContrastInDev('text-muted-foreground','bg-card','ProdComponent')
