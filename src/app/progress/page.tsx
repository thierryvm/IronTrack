...
''
 ...
 <th className="text-left py-2 px-4 font-medium text-foreground">Exercice</th>
 <th className="text-left py-2 px-4 font-medium text-foreground">Groupe musculaire</th>
 <th className="text-left py-2 px-4 font-medium text-foreground">Valeur actuelle</th>
 <th className="text-left py-2 px-4 font-medium text-foreground">Valeur précédente</th>
 <th className="text-left py-2 px-4 font-medium text-foreground">Amélioration</th>
 <th className="text-left py-2 px-4 font-medium text-foreground">Tendance</th>
 </tr>
 </thead>
 <tbody>
 ...
 >
 <td className="py-2 px-4 font-medium text-foreground">{exercise.exercise}</td>
 <td className="py-2 px-4 text-muted-foreground">{exercise.muscle_group}</td>
 ...
 <td className={`py-2 px-4 font-medium ${getTrendColor(exercise.trend)}`}>
 ...
 const bgClass = goal.status !=='Atteint' ?'bg-muted' : (unit ==='kg' ?'bg-orange-50' :'bg-green-50');
 const barClass = goal.status !=='Atteint' ?'bg-gray-300' : (unit ==='kg' ?'bg-primary' :'bg-green-500');
 const textClass = goal.status !=='Atteint' ?'text-gray-600' : (unit ==='kg' ?'text-orange-800' :'text-green-600');
 return (
 <div key={goal.id} className={`p-4 rounded-lg ${bgClass}`}>
 <div className="flex items-center justify-between mb-2">
 <h3 className={`font-medium ${goal.status !=='Atteint' ?'text-gray-600' :'text-foreground'}`}>{goal.exercises?.name}<br /><span className="text-sm text-muted-foreground">{badgeDesc(goal)}</span></h3>
 <div className="flex items-center gap-2">
 <span className={`text-sm ${textClass}`}>{current}/{target} {unit}{extraKg ? ` à ${extraKg} kg` :''}{extraDuration ? ` en ${extraDuration} min` :''}</span>
 <Button variant="ghost" size="sm" title="Éditer" onClick={() => handleEditGoal(goal)} className="ml-2 text-safe-info hover:text-secondary-hover p-1"><Pencil className="h-4 w-4" /></Button>
 ...
 <DialogDescription className="sr-only">Formulaire pour ajouter un nouvel objectif de progression</DialogDescription>
 </DialogHeader>
 ...
 className="bg-primary hover:bg-primary-hover text-white min-h-[44px]"
 >
 ...
 <DialogDescription className="sr-only">Confirmation de suppression d'un objectif de progression</DialogDescription>
 </DialogHeader>
