...
export default function CalendarPage() {
  ...
  // TEMPORAIRE: useEffect désactivé pour éviter les erreurs TypeError
  // useEffect(() => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              ...
              >
{/* TEMPORAIRE: Remplacement par du texte pour éviter DÉFINITIVEMENT l'hydration */}
                <span className="text-sm font-bold">👥</span>
                <span className="hidden sm:inline">Partenaires</span>
                <span className="sm:hidden">Part.</span>
                {partnersWorkouts.length > 0 ? (
                  <span className="bg-white/30 text-white backdrop-blur-sm rounded-full px-2 py-0.5 text-xs font-semibold">
                    {partnersWorkouts.length}
                  </span>
                ) : (
                  <span className="bg-white/20 text-white backdrop-blur-sm rounded-full px-2 py-0.5 text-xs font-bold">
                    0
                  </span>
                )}
              </Button>
              <Button
                ...
                className="bg-orange-600 dark:bg-orange-500 hover:bg-orange-700 dark:hover:bg-orange-600 flex items-center space-x-1 sm:space-x-2 text-sm sm:text-base min-h-[44px] touch-manipulation text-white"
              >
                ...
                <TabsContent value="calendar" className="m-0">
                  ...
                  >
                    ...
                    {['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'].map(day => (
                      <div key={day} className="p-2 text-center text-sm font-medium text-gray-600 dark:text-safe-muted">
                        {day}
                      </div>
                    ))}
                      ...
                      const cell = (
                        <div
                          ...
                          className={`w-full cursor-pointer transition-all touch-manipulation min-h-[44px] rounded-lg ${
                            isCurrentDay ? 'ring-2 ring-orange-500 dark:ring-orange-400' : ''
                          } ${
                            isSelectedDay ? 'ring-2 ring-blue-500 dark:ring-blue-400 bg-blue-50 dark:bg-blue-900/20' : ''
                          }`}
                          role="button"
                          tabIndex={0}
                          onKeyDown={(e) => {
                          ...
                          <div key={workout.id} className={`rounded-lg p-4 border-l-4 ${
                            ...
                            <div className="flex items-start justify-between mb-2">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {workout.duration && (
                                  <span className="flex items-center gap-1 text-sm text-gray-600 dark:text-safe-muted">
                        ...
                        <div className="space-y-3">
                          {(() => {
                            ...
                            if (sessions.length === 0) {
                              return (
                                <div className="text-center py-6 text-gray-600 dark:text-safe-muted">
          ...
          >
                    ...
                    return sessions.map(item => (
                      <div key={item.id} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg flex flex-col gap-1">
                        ...
                        {item.participants && item.participants.length > 0 && item.participants[0] && (
                          <span className="ml-2 text-xs text-gray-600 dark:text-safe-muted flex items-center gap-1">
                            <span className="text-blue-600">👥</span>
                            Partagée avec {item.participants.map(p => p.name || 'Utilisateur IronTrack').join(', ')}
                          </span>
                        )}
                        {item.exercises && item.exercises.length > 0 && (
                          <div className="mt-1 text-xs text-gray-600 dark:text-safe-muted">
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl shadow-md p-4 lg:p-6">
              ...
              <div className="space-y-5">
                ...
                <div className="flex items-center gap-3">
                  <Target className="h-6 w-6 text-safe-info" />
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <div className="flex flex-col">
                    <span className="text-xs text-gray-600 dark:text-safe-muted">Total séances</span>
                    <span className="font-bold text-gray-900 dark:text-gray-100 text-lg">{totalSeances}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-xs text-gray-600 dark:text-safe-muted">Durée moyenne</span>
                    <span className="font-bold text-gray-900 dark:text-gray-100 text-lg">{moyenneDuree} min</span>
                  </div>
                  <div className="flex flex-col col-span-2">
                    <span className="text-xs text-gray-600 dark:text-safe-muted">Séance la plus longue</span>
                    <span className="font-bold text-orange-700 text-base">{maxDuree > 0 && seanceMax ? `${seanceMax.name} (${maxDuree} min)` : '-'}</span>
                  </div>
                  <div className="flex flex-col col-span-2">
                    <span className="text-xs text-gray-600 dark:text-safe-muted">Séance la plus courte</span>
            ...
            <div className="p-4 space-y-6">
              ...
              <Card className="p-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Target className="h-5 w-5 text-safe-info" />
                ...
                <div className="mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 p-3 rounded">
                      <span className="text-xs text-gray-600 dark:text-safe-muted block">Total séances</span>
                      <span className="font-bold text-gray-900 dark:text-gray-100 text-lg">{totalSeances}</span>
                    </div>
                    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 p-3 rounded">
                      <span className="text-xs text-gray-600 dark:text-safe-muted block">Durée moyenne</span>
                      <span className="font-bold text-gray-900 dark:text-gray-100 text-lg">{moyenneDuree} min</span>
                    </div>
                  </div>
                  ...
                  {maxDuree > 0 && seanceMax && (
                    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 p-3 rounded mb-2">
                      <span className="text-xs text-gray-600 dark:text-safe-muted block">Séance la plus longue</span>
                  ...
                  {minDuree > 0 && seanceMin && (
                    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 p-3 rounded mb-3">
                      <span className="text-xs text-gray-600 dark:text-safe-muted block">Séance la plus courte</span>
