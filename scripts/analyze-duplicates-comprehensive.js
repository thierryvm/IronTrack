// Script d'analyse complète des doublons d'exercices et de leur impact
// Ce script utilise l'API client Supabase pour analyser les données

const { createClient } = require('@supabase/supabase-js');

const supabase = createClient(
  'https://taspdceblvmpvdjixyit.supabase.co',
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRhc3BkY2VibHZtcHVkaml4eWl0Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MTc1NDc3NiwiZXhwIjoyMDY3MzMwNzc2fQ.K32qBRR6E5gmNFtSNYxaKKH9kcYbF0Wc73ewBwq3QkI'
);
...
async function analyzeExerciseDuplicates() {
  ...
  try {
    // 1. Récupérer tous les exercices avec leurs informations complètes
    const { data: exercises, error: exercisesError } = await supabase
      ...
      .select(`
        ...
        muscle_groups:muscle_group_id(name)
      `)
    ...
    // 4. Analyser les doublons en détail
    if (duplicates.length > 0) {
      ...
      for (let i = 0; i < duplicates.length; i++) {
        ...
        console.log(`\n${i + 1}. "${dup.exercises[0].name}" (${dup.exercises.length} versions)`);
        
        dup.exercises.forEach((ex, index) => {
          const equipment = ex.equipment?.name || 'N/A';
          const muscleGroup = ex.muscle_groups?.name || 'N/A';
          ...
          console.log(`   ${marker} ID ${ex.id} | ${createdDate} | ${ex.type} | ${equipment} | ${muscleGroup} | User: ${ex.user_id}`);
        });
      }
    }
    ...
    duplicates.forEach(dup => {
      ...
      if (affectedPerformances.length > 0) {
        performanceFragmentation[dup.name] = {
        ...
        duplicateIds.forEach(id => {
          ...
          performanceFragmentation[dup.name].distributionByVersion[id] = perfsForThisVersion.length;
        });
    ...
    // 6. Identifier les utilisateurs affectés
    ...
    Object.values(performanceFragmentation).forEach(fragData => {
      Object.entries(fragData.distributionByVersion).forEach(([exerciseId, count]) => {
        ...
        perfsForExercise.forEach(perf => {
          ...
          userImpact[perf.user_id].affectedExercises.add(perf.exercises?.name);
        });
      });
    });
    ...
    // 7. Recommandations de stratégie
...
module.exports = { analyzeExerciseDuplicates };