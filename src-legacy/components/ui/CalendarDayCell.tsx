...
const CalendarDayCell: React.FC<CalendarDayCellProps> = ({ date, sessions, isSelected = false}) => {
 ...
 return (
 <div className={`relative h-20 sm:h-24 p-1 sm:p-2 border rounded-lg bg-card flex flex-col overflow-hidden transition-all duration-200 ${
 isSelected 
 ...
 <button
 className="flex items-center px-2 py-2 text-xs text-white font-medium rounded-md shadow-sm cursor-pointer transition-all duration-150 hover:scale-[1.02] active:scale-95 touch-manipulation w-full"
 style={{
 ...
 {sessions.length > 5 && (
 <span className="text-sm text-muted-foreground ml-1 min-w-[20px] text-center">+{sessions.length - 5}</span>
 )}
 </div>
 </div>
 ) : (
 ...
 <button
 className="flex-1 min-h-[32px] flex items-center px-2 py-2 text-xs text-white font-medium rounded-md shadow-sm cursor-pointer transition-all duration-150 hover:scale-[1.02] active:scale-95 touch-manipulation"
 style={{
 background: session.color ||'linear-gradient(90deg, #ff9800 0%, #ffb347 100%)',
 minWidth: 0,
 overflow:'hidden',
 ...
 <PopoverContent className="w-80 max-w-[90vw] p-2 max-h-[80vh] overflow-y-auto">
 ...
 <div
 key={session.id}
 className="p-2 rounded-lg border bg-gradient-to-r from-orange-50 to-orange-100 border-orange-200"
 >
 ...
 >
 <div 
 className="bg-card border border-border rounded-lg shadow-2xl p-4 w-80 max-w-full relative"
 ...
 >
 <button 
 className="absolute top-2 right-2 text-gray-700 hover:text-gray-700 p-1 rounded hover:bg-gray-100 transition-colors min-h-[44px] min-w-[44px] touch-manipulation" 
 onClick={(e) => {
 e.stopPropagation();
 ...
 <div className="text-sm text-muted-foreground mb-2">{formatDateTime(mobileSessionDetail.scheduled_date, mobileSessionDetail.time)}</div>
 )}
 ...
 <span className="text-xs text-muted-foreground">Partagée par</span>
 <Avatar
 ...
 />
 <span className="text-xs text-muted-foreground">{mobileSessionDetail.participants[0].name}</span>
 </div>
 )}
 </div>
 </div>
