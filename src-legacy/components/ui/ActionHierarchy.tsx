...
export function ActionHierarchy({ 
 ...
 switch (action.type) {
 case'primary':
 return `${baseClasses} py-2 px-4 rounded-lg shadow-md hover:shadow-lg focus:ring-2 focus:ring-offset-2 ${getPrimaryVariantClasses(action.variant)}`
 
 case'secondary':
 return `${baseClasses} py-2 px-4 rounded-lg focus:ring-2 focus:ring-offset-2 ${getSecondaryVariantClasses(action.variant)}`
 
 case'tertiary':
 return `${baseClasses} py-2 px-2 rounded-md text-sm focus:ring-1 ${getTertiaryVariantClasses(action.variant)}`
 ...
 default:
 return'bg-gradient-to-r from-orange-600 to-red-500 text-white hover:from-orange-600 hover:to-red-600 focus:ring-primary'
}
}
