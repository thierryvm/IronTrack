...
const FoodSearchAutocomplete: React.FC<FoodSearchAutocompleteProps> = ({
 ...
 className="w-full pl-8 pr-8 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
 autoComplete="off"
 spellCheck="false"
 />
 ...
 <button
 onClick={clearSearch}
 className="absolute inset-y-0 right-0 pr-2 flex items-center hover:text-foreground text-muted-foreground"
 >
 ...
 {/* Message d'erreur */}
 {error && (
 <div className="absolute z-10 w-full mt-1 p-2 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">
 {error}
 </div>
 )}
 ...
 <Package className={`h-6 w-6 text-muted-foreground ${food.image_url ?'hidden' :''}`} />
 </div>
 ...
 <div className="absolute z-20 w-full mt-1 bg-card border border-border rounded-lg shadow-lg p-4 text-center text-muted-foreground">
